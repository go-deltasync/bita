...
// buzHash is a port of bita's BuzHash rolling hash. See
...
var buzHashBaseTable = [256]uint32{
...
func newBuzHash(window int) *buzHash {
	...
	for i := range buzHashBaseTable {
		b.table[i] = buzHashBaseTable[i] ^ buzHashSeed
	}
	return b
}

func (b *buzHash) init(in byte) {
	if !b.windowFull {
		v := b.table[in]
...
func (b *buzHash) initDone() bool { return b.windowFull }

func (b *buzHash) input(in byte) {
